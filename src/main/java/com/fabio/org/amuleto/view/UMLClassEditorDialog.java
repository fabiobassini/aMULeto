 ...
 * costruttori e i metodi della classe, in base all'input fornito dall'utente.
 */
public class UMLClassEditorDialog extends JDialog {
  ...
  /**
   * Costruttore del dialogo. Viene impostato il titolo in base al nome della classe e il dialogo
   * viene reso modale.
   *
  ...
  private void initComponents() {
    ...
    formPanel.add(
        labeledPanel(
            "Attributi (uno per riga, formato: visibilità tipo nome):",
  ...
  /**
   * Aggiorna l'istanza UMLClassShape con i dati immessi dall'utente nelle aree di testo. Il metodo
   * svuota le liste degli attributi, costruttori e metodi, e le riempie con i nuovi valori, parsati
   * in base a un formato predefinito.
   */
  private void updateUMLClass() {
    ...
    for (String line : attributesArea.getText().split("\\n")) {
      ...
      if (!line.isEmpty()) {
        ...
        if (colonIndex > 0) {
          String beforeColon =
              line.substring(0, colonIndex).trim(); // Parte prima dei due punti (es. "+ id")
          String afterColon =
              line.substring(colonIndex + 1).trim(); // Parte dopo i due punti (es. "int")
    ...
    for (String line : methodsArea.getText().split("\\n")) {
      ...
      if (!line.isEmpty()) {
        ...
        if (colonIndex > 0) {
          String beforeColon =
              line.substring(0, colonIndex).trim(); // Esempio: "+ metodo(parametri)"
  ...
  /**
   * Crea un pannello etichettato. Viene creato un pannello con un'etichetta in alto e il componente
   * fornito al centro.
   *
   * @param labelText Il testo dell'etichetta.
   * @param comp Il componente da visualizzare sotto l'etichetta.
   * @return Un pannello con layout BorderLayout contenente l'etichetta e il componente.
   */

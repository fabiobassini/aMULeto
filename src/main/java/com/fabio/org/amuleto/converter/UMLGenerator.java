...
/**
 * Classe che fornisce metodi statici per generare un diagramma UML a partire dal codice Java. Il
 * diagramma viene generato analizzando ricorsivamente i file Java in una directory e costruendo una
 * rappresentazione testuale in stile PlantUML.
 */
public class UMLGenerator {

  /**
   * Genera il diagramma UML in formato testo (PlantUML) a partire da tutti i file Java contenuti in
   * una directory (ricorsivamente) e scrive il risultato nel file di output. I tipi vengono
   * raggruppati per package e vengono esclusi i file appartenenti al generatore stesso.
   *
   * @param sourceDir La directory sorgente contenente i file Java.
   * @param outputFile Il file in cui verrà scritto il diagramma UML in formato testo.
  ...
  /**
   * Genera direttamente un file vettoriale (SVG o PDF) a partire dai file Java nella directory.
   *
   * @param sourceDir La directory sorgente contenente i file Java.
   * @param outputVectorFile Il file di output in cui verrà salvato il diagramma (con estensione
   *     .svg o .pdf).
   * @param vectorFormat Il formato vettoriale desiderato (ad esempio FileFormat.SVG).
  ...
  /**
   * Processa una classe o interfaccia, generando la sua definizione in UML e le relative relazioni
   * (ereditarietà, implementazioni, associazioni, dependency).
   *
   * @param cid Il tipo (classe o interfaccia) da processare.
   * @param uml Il StringBuilder in cui verrà aggiunta la definizione della classe.
   * @param relationships Il StringBuilder in cui verranno aggiunte le relazioni.
   * @param definedTypes L'insieme dei nomi dei tipi definiti, per il controllo delle dependency.
   */
  private static void processType(
    ...
    if (!isAssociationClass) {
      ...
      if (assocClasses.size() == 2) {
        ...
        relationships.append(className).append(" ..> ").append(classA).append(" : association\n");
        relationships.append(className).append(" ..> ").append(classB).append(" : association\n");
      }
    }
  }
  ...
  /**
   * Processa un campo per determinare se genera un'associazione (se il tipo non è primitivo).
   ...
   * @param uml Il StringBuilder in cui aggiungere la definizione della classe.
   ...
   */
  static void processAssociation(
      Type fieldType,
  ...
  /**
   * Restituisce una lista di tutti i file Java (con estensione .java) presenti in una directory,
   * effettuando una ricerca ricorsiva.
   *
